...
while StartStopForm
    ...
    % else
    %     % TEST MODE:
    %     % Ignore Myo gyro for now.
    %     % This forces one joint to move so we can find the simulator rotation joint.
    % 
    %     testJoint = 5;   % Try 7 first. If nothing moves, change to 6, 5, 4, etc.
    % 
    %     q = q_home;
    %     q(testJoint) = 1.5 * sin(elapsedTime);
    % 
    %     grip = gripMin;
    % 
    %     fprintf('\r[%5.1f] TEST JOINT %d | q=%6.2f', ...
    %         elapsedTime, testJoint, q(testJoint));
    % end
